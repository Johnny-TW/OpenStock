...
#### 必須列出
- 新增/移除功能模組或頁面
- 破壞性變更（API 介面改動、套件替換、配置結構變更）
- 重要邏輯修改（認證流程、路由守衛、資料流）
- 套件新增/移除/版本升級（標示版本號）
- CI/CD 流程變更

#### 可以列出（視重要性）
- 元件重構或重新命名
- 配置檔新增/修改（biome.json、tsconfig 等）
- 文件更新（若為配套變更，一句帶過即可）

#### 不需列出
- Formatter/Linter 自動修復的格式變更（如引號、縮排、import 排序）
- 單純的 unused import 移除
- 樣式微調（除非是主題改版）
- lock 檔異動（pnpm-lock.yaml）
- 與主要變更無直接關聯的 typo 修正
