...
import {
  ArrowLeft, ExternalLink, Loader2, TrendingUp, TrendingDown,
  PieChart, Target, Building2, AlertCircle,
} from "lucide-react"
...
export default function StockDetailClient({ symbol }: StockDetailClientProps) {
  ...
  const stockDetailLoaded = useAppSelector(
    (state) => state.stock?.stockDetailLoaded ?? false
  )
  ...
  const detail: StockDetailDto | null =
    rawDetail && rawDetail.symbol === symbol ? rawDetail : null
              ...
              <div className={styles.chartSection}>
                <div className={styles.chartHeader}>
                  <div className={styles.stockTitle}>
                    {detail.name} · {formatExchange(detail.symbol)}
                    {detail.sector && (
                      <span className={styles.sectorBadge}>{detail.sector}</span>
                    )}
                    {detail.industry && (
                  ...
                  <div className={styles.quickStats}>
                    ...
                    <div className={styles.statItem}>
                      ...
                      <span className={`${styles.statValue} ${styles.up}`}>{formatPrice(detail.high)}</span>
                    </div>
                    <div className={styles.statItem}>
                      <span className={styles.statLabel}>最低</span>
                      <span className={`${styles.statValue} ${styles.down}`}>{formatPrice(detail.low)}</span>
                    </div>
                    <div className={styles.statItem}>
            ...
            <div className={styles.rightCol}>
              <div className={styles.infoSection}>
                ...
                <div className={styles.metricGrid}>
                  ...
                  <div className={styles.metricCell}>
                    ...
                    <span className={styles.metricValue}>{detail.peRatio ? `${detail.peRatio.toFixed(1)}x` : "-"}</span>
                  </div>
                  <div className={styles.metricCell}>
                    <span className={styles.metricLabel}>預估本益比</span>
                    <span className={styles.metricValue}>{detail.forwardPE ? `${detail.forwardPE.toFixed(1)}x` : "-"}</span>
                  </div>
                  <div className={styles.metricCell}>
                  ...
                  <div className={styles.metricCell}>
                    ...
                    <span className={styles.metricValue}>{detail.dividendYield ? `${detail.dividendYield.toFixed(2)}%` : "-"}</span>
                  </div>
                  <div className={styles.metricCell}>
                    <span className={styles.metricLabel}>市淨率 P/B</span>
                    <span className={styles.metricValue}>{detail.pbRatio ? `${detail.pbRatio.toFixed(1)}x` : "-"}</span>
                  </div>
                  <div className={styles.metricCell}>
                    <span className={styles.metricLabel}>ROE</span>
                    <span className={`${styles.metricValue} ${detail.returnOnEquity > 0.15 ? styles.highlight : ""}`}>
                  ...
                  <div className={styles.metricCell}>
                    ...
                    <span className={`${styles.metricValue} ${detail.revenueGrowth > 0 ? styles.up : detail.revenueGrowth < 0 ? styles.down : ""}`}>
                      {detail.revenueGrowth ? `${detail.revenueGrowth > 0 ? "+" : ""}${(detail.revenueGrowth * 100).toFixed(1)}%` : "-"}
                    </span>
                  </div>
                  <div className={styles.metricCell}>
              ...
              <div className={styles.infoCard}>
                ...
                <div className={styles.cardBody}>
                  ...
                  <div className={styles.infoRow}>
                    <span>盈餘成長率</span>
                    <span className={detail.earningsGrowth > 0 ? styles.up : detail.earningsGrowth < 0 ? styles.down : ""}>
                  ...
                  <div className={styles.infoRow}>
                    <span>在外流通股數</span>
                    <span>{detail.sharesOutstanding ? formatVolume(detail.sharesOutstanding) : "-"}</span>
                  </div>
                </div>
              </div>
                ...
                <div className={styles.cardBody}>
                  ...
                  {detail.targetMeanPrice > 0 && (
                    <>
                      ...
                      <div className={styles.infoRow}>
                        <span>分析師目標均價</span>
                        <span className={styles.highlight}>{formatPrice(detail.targetMeanPrice)}</span>
                      </div>
                      <div className={styles.infoRow}>
                        <span>目標價區間</span>
                        <span>
                      ...
                      <div className={styles.infoRow}>
                        ...
                        <span className={styles.recommendation}>
                          {RECOMMENDATION_MAP[detail.recommendationKey] || detail.recommendationKey || "-"}
                        </span>
                      </div>
                      <div className={styles.infoRow}>
                        <span>分析師人數</span>
